...
# If I want to check whether a number <n> is prime...
#       ^ (you know a fixed <n> in advance)
#   all x: Int | (x > 1 and x < n) => remainder[n,x] != 0
# Forge will compile this into booleans.

# efficiency 
  # we could refine the predicate (e.g., evens)
  # no "ordering" -- it all gets compiled and solved at once
  # compiled OUT a bunch of useful domain-specific intuition
  # limited by bitwidth
...
def demoUninterpreted():
    ...
    # To typecheck this well, Python would need to look at the type of the function domain.
    # But the sort arguments are _runtime values_... So we will regretfully suppress this error.
    s.add(And(b > a, f(b) < f(a))) # type: ignore
    if s.check() == sat:        
        print(s.model()) 
        print(s.model().evaluate(f(a)))
        print(s.model().evaluate(f(b)))
        print(s.model().evaluate(f(1000000)))
#if __name__ == "__main__": demoUninterpreted()
...
def demoReals():
    ...
    if result == sat:
        ...
        print(result)
#if __name__ == "__main__": demoReals()
...
def demoFactoringIntWithUniversal():
    ...
    s.add(ForAll(xi, (xi + r1i) * (xi + r2i) 
                     == (xi * xi) + (198 * xi) - 39483)) # type: ignore
    result = s.check()
    if result == sat:
        print(s.model())    
    else: 
...
def demoFactoringReals():
    ...
    if result == sat:
        ...
        print(result)
#if __name__ == "__main__": demoFactoringReals()
...
def demoFactoringRealsUnsat():
    ...
    if result == sat:
        ...
        print(s.unsat_core())
#if __name__ == "__main__": demoFactoringRealsUnsat()
