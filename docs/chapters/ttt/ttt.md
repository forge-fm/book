...
Here's a useful comparison to help reinforce the difference (with thanks to [Daniel Jackson](https://en.wikipedia.org/wiki/Daniel_Jackson_(computer_scientist)), who first shared this analogy with me):
- An empty program **does nothing** (because there are no instructions to run).
- An empty model **allows everything** (because there are no rules).

## Modeling Tic-Tac-Toe Boards

What are the essential concepts in a game of tic-tac-toe?

!!! tip "Modeling Methodology"
    ...
    These steps will get us to a point we can begin to iterate: working to **validate and/or refine** the model as we go. It's rare that a model will do everything you need (and do so correctly) on the first try&mdash;this process is agile, and frequently loops back.


!!! warning "Well-formedness vs. domain predicates"
    Why make this distinction between well-formedness and domain predicates? Because the first should always hold in _any_ instance Forge considers, but the other may or may not hold in a valid instance. In fact, we might want to use Forge to _verify_ that a domain predicate always holds! And if we've told Forge that any instance that doesn't satisfy it is garbage, Forge won't find us such an instance.
...
!!! tip "There is no heap in Forge unless you model it yourself."
...
* Forge integers aren't true mathematical integers, but are bounded by a bitwidth we give whenever we run the tool. This resembles what you might have seen in programming languages with a "maximum integer": with 32 bits, you can represent around 4 billion values. Forge has far fewer bits to work with, but there are still more than we really need for tic-tac-toe. So we should be careful here. We want a classical 3-by-3 board with indexes of (say) `0`, `1`, and `2`, not a board where (e.g.) row `-5`, column `-1` is a valid location. 

We'll call these _well-formedness_ constraints. They aren't innately enforced by our `sig` declarations, but we'll always want Forge to enforce them, so that it doesn't find or consider such "garbage instances". Let's write a _wellformedness predicate_:
...
!!! tip "Predicates are declarative"
...
In Forge, `example`s are automatically run whenever your model executes. They describe basic intent about a given predicate, and serve as a basic kind of test case. Let's write two examples in Forge:
...
!!! warning "Test in both directions"
...
<!-- **TODO: replace img with one matching the table view**
...
!!! note "Why `Board3` when there's only one board?"
...
The `{row, col: Int | ...}` syntax is called a _set comprehension_. A set comprehension defines a set. We're defining the set of row-column pairs where the board contains one of the player marks. The `#` operator gives the size of these sets, which we then compare. We're work more with defining our own sets later, but for now we'll _only_ use them to count elements.
...
!!! warning "Use `add` for addition, not `+`"
...
// "is sat" means "is satisfiable": Forge can find an instance.
```
