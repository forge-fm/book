...
-- FROM EXERICSE: why separate predicates? (because guards differ)
--   guards: "this must be true for the transition to be taken"
--   actions: "this is the effect of the transition being taken"
--   frame conditions: no-op actions that keep unaffected state constant
...
-- FROM EXERCISE: yes, the above is only true if one of the transitions we defined holds
...
-- MAKE SURE YOU TEST VS. BOTH UNDER- AND OVER-CONSTRAINT

-- FROM EXERCISE: "exactly 1" prevents 0
--   we can get away with "exactly 1 IntArray" for the is-sat tests below 
--   because the array itself never changes

-- Some "is satisfiable" tests. These test for consistency, possibility, non-vacuity;
--   may reveal OVER-constraint bugs.
test expect {
...
--   May detect UNDER-constraint bugs.
...
pred transitionFromGoodState[pre, post: SearchState] {
  bsearchInvariant[pre]
  anyTransition[pre, post]
  safeArraySize[pre.arr]
}
assert all s1, s2: SearchState | transitionFromGoodState[s1, s2] 
