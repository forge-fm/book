...
!!! warning "Don't let the perfect be the enemy of the good."
...
A pair of students at Brown did an independent study on [modeling crypto protocols](http://cs.brown.edu/~tbn/publications/ssdnk-fest21-forge.pdf), using the tools you'll learn here. Here's an example picture, generated by their model, of the flaw in the Needham-Schroeder protocol:
...
For better or worse, The shape of engineering is changing. Lots of people are excited, scared, or both about large language models and tools that use them, like [Github Copilot](https://github.com/features/copilot) and [Claude Code](https://code.claude.com/docs/en/overview). This book won't teach you how to use generative AI, so it's reasonable to wonder: _why learn from this book, instead of reading yet another book on another (deservedly) hot topic, like machine learning?_
...
!!! note "Another Example: Types"
    **Types** are a form of lightweight specification. They are also one of the most widely used, valuable formal-methods contributions in history. They are so common that nobody even thinks of them as "formal methods" anymore. But inside the Java compiler, you'll find a little proof checker, and when you work with generics, you're doing a kind of algebra. [Rust's Chalk trait solver](https://rustc-dev-guide.rust-lang.org/traits/chalk.html) is based on logic programming and constraints&mdash;ideas that you'll learn about if you study formal methods. 
...
<!-- * [Dafny](https://dafny.org/), a programming language with verification built in. -->
